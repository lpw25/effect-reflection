...
Terms in the free algebras of the sum of two effects
...
\subsection{Effect sums and inductive types}
...
\label{lemma:sums}
  ...
  \end{equation*}
\end{lemma}
where $\mu X.F(X)$ is the intial algebra of a suitable endofunctor $F$,
and $F_\Sigma$ is the endofunctor on $\setc$ for which $T_{\Sigma}$ is
the free monad.

Effect reification works by exposing $\mu X.T_L(S + F_\Sigma(X))$ to the
user via an inductive type. In particular, given $\Sigma$ and a
presheave $P$, we can define the following initial algebra:
\begin{align*}
...
\begin{align*}
\arrow{\yoneda{L}}{\mon{(\glob P)}}(L')
  &\cong \homset{\pshlawcop}{\yoneda{L'} \times \yoneda{L}}{\mon{(\glob P)}} \\
  &\cong \homset{\pshlawcop}{\yoneda{L' + L}}{\mon{(\glob P)}} \\
  &\cong \mon{(\glob P)}(L' + L) \\
  &\cong T_{(L' + L)}(P(L'))
\end{align*}

In the case where $L$ is a signature $\Sigma$, we can further apply our
variation of Lemma \ref{lemma:sums}, and observing that all the steps in
this proof were functorial, produce this isomorphism:
\begin{equation*}
  \arrow{\yoneda{\Sigma}}{\mon{(\glob P)}} \cong \mon{(\ind{\Sigma}(\glob P))}
\end{equation*}
which we call $\reifyc{\Sigma}{}$.

$\reifyc{\Sigma}{}$ directly gives us our semantics for reification.
\begin{align*}
